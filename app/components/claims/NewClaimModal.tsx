...
// Update the props interface to include onClaimSubmitted
interface NewClaimModalProps {
  isOpen: boolean;
  onClose: () => void;
  benefits: BenefitSummary[];
  onClaimSubmitted?: () => void; // Add this optional prop
}
...
export default function NewClaimModal({ 
  ...
  onClose, 
  benefits,
  onClaimSubmitted // Add this to destructured props
}: NewClaimModalProps) { // Use the interface here
  const { theme } = useTheme();
  const router = useRouter();
  const [user, setUser] = useState<User | null>(null);
  const [selectedBenefit, setSelectedBenefit] = useState<BenefitSummary | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Call the onClaimSubmitted callback if provided
      if (onClaimSubmitted) {
        onClaimSubmitted();
      }
