...
function normalizeStatusForFilter(status: string): string {
  ...
  // Direct mapping to match attendance_statuses table
  const statusUpper = status.toUpperCase();
...
export default function AttendanceTab({
  ...
  // const employeeStatuses = ['Active', 'Inactive', 'Resigned'];
  const positions = useMemo(() => (filterOptions as any)?.positions || [], [filterOptions]);
  // REMOVED: const employmentTypes = useMemo(() => (filterOptions as any)?.employment_types || [], [filterOptions]);
  // REMOVED: const nationalities = useMemo(() => (filterOptions as any)?.nationalities || [], [filterOptions]);
  // REMOVED: const jobLevels = useMemo(() => (filterOptions as any)?.job_levels || [], [filterOptions]);
  ...
  }, [filters, baseFilters, isFetching, isLoading]);
...
// Handle export (reusable, UI-agnostic)
const handleExport = async () => {
  if (isExporting || selectedColumns.length === 0) return;
  ...
  try {
    ...
    console.log('🔍 [ATTENDANCE EXPORT DEBUG] Starting export with:', {
      exportScope,
      currentPage: page,
      limit: limit,
    ...
    // Export scope - FIXED: Different parameters for current vs all
    if (exportScope === 'current') {
      // Current page only
      queryParams.append('page', String(page));
      queryParams.append('limit', String(limit));
    ...
    const url = `${API_BASE_URL}/api/attendance/export?${queryParams.toString()}`;
    ...
    let filename;
    if (exportScope === 'current') {
      filename = `attendance_export_page${page}_${format(new Date(), 'yyyyMMdd_HHmmss')}.xlsx`;
    } else {
      filename = `attendance_export_all_${format(new Date(), 'yyyyMMdd_HHmmss')}.xlsx`;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className={`p-4 rounded-lg ${theme === 'light' ? 'bg-yellow-50 border border-yellow-200' : 'bg-yellow-900/20 border border-yellow-800'}`}>
          ...
          <div className="text-xs space-y-1">
            ...
            <div><strong>isFetching:</strong> {isFetching.toString()}</div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/20 backdrop-blur-sm">
          <div className={`p-6 rounded-xl shadow-2xl ${theme === 'light' ? 'bg-white' : 'bg-slate-800'}`}>
            <div className="flex flex-col items-center gap-4">
              <div className="loading loading-spinner loading-lg text-primary"></div>
              <span className={`font-medium ${theme === 'light' ? 'text-gray-700' : 'text-slate-300'}`}>
                {isLoading ? 'Loading records...' : 'Updating records...'}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Advanced Filters - ONLY SHOW REQUIRED FILTERS */}
      {isFilterOpen && (
        <div className={`relative rounded-xl shadow-lg border ${theme === 'light' ? 'bg-white border-slate-200' : 'bg-slate-800 border-slate-700'}`}>
          <div className="px-6 py-5 border-b bg-slate-50">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
