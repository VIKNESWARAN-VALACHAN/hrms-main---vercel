...
// ]);//};
  ...
  const renderDocumentSection = (documentType: string) => {
    ...
    return (
      <div key={documentType} className="mb-8">
        <div className={`border rounded-lg p-4 ${theme === 'light' ? 'border-gray-200 bg-white' : 'border-slate-600 bg-slate-800'}`}>
          <div className='flex justify-between flex-row mb-2'>
            <h3 className={`text-lg font-semibold ${theme === 'light' ? 'text-gray-900' : 'text-slate-100'}`}>{typeConfig.label}</h3>
            {mode !== "view" && (
              <button
                type="button"
          ...
          <div className={`items-center p-4 border-2 border-dashed rounded-lg ${theme === 'light' ? 'border-gray-300 bg-gray-50' : 'border-slate-600 bg-slate-700/50'}`}>
            <div className='flex justify-between flex-row'>
              <div>
                <p className={`text-sm text-center mb-2 ${theme === 'light' ? 'text-gray-600' : 'text-slate-400'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {documents.map((doc, index) => (
                <div key={`${doc.name}-${index}`} className={`card border ${theme === 'light' ? 'bg-white border-gray-200' : 'bg-slate-800 border-slate-600'}`}>
                  <div className="card-body p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <h4 className={`card-title text-sm mb-1 truncate ${theme === 'light' ? 'text-gray-900' : 'text-slate-100'}`} title={doc.name}>{doc.name}</h4>
                        {doc.uploadDate && (
                      ...
                      {mode !== "view" && (
                        <button
                          type="button"
                          className={`btn btn-ghost btn-xs text-error ${theme === 'light' ? 'hover:bg-red-50' : 'hover:bg-red-900/20'}`}
                    ...
                    {doc.url && (
                      <div className="mt-2">
                        <button
                          ...
                          className={`btn btn-sm w-full ${theme === 'light' ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  }, [employeeId, mode, selectedFiles.length, previousEmployeeId, handleUploadClick]);
    //}, [employeeId, mode, selectedFiles.length]);
    ...
    <div className="space-y-6">
      ...
      {showDeleteConfirm && (
        <div className="modal modal-open">
          <div className={`modal-box ${theme === 'light' ? 'bg-white' : 'bg-slate-800'}`}>
            <h3 className={`font-bold text-lg ${theme === 'light' ? 'text-gray-900' : 'text-slate-100'}`}>Confirm Delete</h3>
            <p className={`py-4 ${theme === 'light' ? 'text-gray-700' : 'text-slate-300'}`}>
              Are you sure you want to delete "{showDeleteConfirm.document.name}"?
              This action cannot be undone.
            </p>
            <div className="modal-action">
              <button
                className={`btn btn-ghost ${theme === 'light' ? 'hover:bg-gray-100' : 'hover:bg-slate-700'}`}
      ...
      {mode !== "view" && selectedFiles.length > 0 && mode === "edit" && (
        <div className="flex justify-end">
          <button
            type="button"
            className={`btn ${theme === 'light' ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}
