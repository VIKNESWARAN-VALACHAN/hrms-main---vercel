...
import { BsXCircle } from "react-icons/bs";
import { BsEye } from "react-icons/bs";
import { BsCheckCircle as BsCheck } from "react-icons/bs";
import { calculateDateInUTC, calculateDuration, getBadgeClass, formatToMalaysiaTime ,formatDateOnly ,formatDateDDMMMYYYY  } from '../utils/utils';
import EmployeeDocumentManager from '../components/EmployeeDocumentManager';
import NotificationToast from '../components/NotificationToast';
import { useNotification } from '../hooks/useNotification';
...
interface Filters {
  ...
  employee_id: string[];
  leave_type_id: string[];
  application_date_range: string[]; // Keep this for filter options
  leave_date_range: string[]; // Add this for leave date range filter
}
...
// --- ProfessionalMultiSelectFilter Component ---
...
// Add this component before your main AdminLeaveRequest component
    ...
    <div className="form-control w-full">
      ...
      {(minDate || maxDate) && (
        <div className={`text-xs mt-1 ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'}`}>
          Available: {minDate ? formatDateDDMMMYYYY(minDate) : 'No min'} - {maxDate ? formatDateDDMMMYYYY(maxDate) : 'No max'}
        </div>
      )}
    </div>
  );
...
// --- Main AdminLeaveRequest Component ---
...
const AdminLeaveRequest = () => {  
  ...
  // Updated Filter State to support multi-select
...
const fetchFilterOptions = useCallback(async () => {
  try {
    ...
    // Map frontend filters to backend expected names
    const selectedFilters = {
      company_id: filters.company_id, // Changed from company_ids
      department_id: filters.department_id, // Changed from department_ids
      position: filters.position,
      type: filters.type,
      ...
      application_date_range: filters.application_date_range, 
      searchTerm: searchTerm
    };
    
    console.log('📤 Sending selectedFilters:', selectedFilters);
    ...
    console.log('📥 Received options:', response.data.options);
    
    if (response.data.success) {
      setFilterOptions(response.data.options);

        // Store the date range limits
        if (response.data.options.application_date_range) {
          setDateRangeLimits(prev => ({
            ...prev,
            application: {
              min: response.data.options.application_date_range.min_date || '',
              max: response.data.options.application_date_range.max_date || ''
            }
          }));
        }
        
        if (response.data.options.leave_date_range) {
          setDateRangeLimits(prev => ({
            ...prev,
            leave: {
              min: response.data.options.leave_date_range.min_date || '',
              max: response.data.options.leave_date_range.max_date || ''
            }
          }));
        }
    }
...
}, [filters, searchTerm]); // Add searchTerm dependency
...
// Then in useEffect that calls fetchFilterOptions:
useEffect(() => {
  if (user) {
  ...
  try {
    ...
    // 3. Make sure response.data.data is an array
    if (response.data.success && Array.isArray(response.data.data)) {
      ...
      setLeaveRequests([]); // Set to empty array as fallback
      setError('Invalid data format received from server');
    }
  } catch (err) {
    console.error('Error fetching leave requests:', err);
    setError('Failed to load leave requests. Please try again.');
    setLeaveRequests([]); // Reset to empty array on error
  ...
  try {
    ...
    if (Array.isArray(response.data)) {
      ...
      setLeaveTypesByEmployeeId(response.data.data);
    } else if (response.data?.success && Array.isArray(response.data.data)) {
      // If response has success flag
      setLeaveTypesByEmployeeId(response.data.data);
    } else {
      console.error('Unexpected response format:', response.data);
      setLeaveTypesByEmployeeId([]); // Set to empty array as fallback
      showNotification('Failed to load leave types. Invalid data format.', 'error');
    }
  } catch (err) {
    console.error('Error fetching leave types:', err);
    setLeaveTypesByEmployeeId([]); // Set to empty array on error
    showNotification('Failed to load leave types. Please try again.', 'error');
  }
};
 ...
 const fetchLeaveBalances = async (employeeId: number) => {
  try {
    console.log('📥 Fetching balances for employee:', employeeId);
    ...
    // Format the balances to ensure consistent structure
    const formattedBalances = balances.map((balance: any) => ({
      id: balance.id || 0,
      employee_id: balance.employee_id || employeeId.toString(),
      leave_type_id: balance.leave_type_id || balance.leave_type_id || '',
      leave_type_name: balance.leave_type_name || balance.name || balance.leave_type || 'Unknown',
      year: balance.year || new Date().getFullYear(),
      total_days: balance.total_days || balance.total || 0,
      used_days: balance.used_days || balance.used || 0,
      remaining_days: balance.remaining_days || balance.remaining || 0,
      accrual_days: balance.accrual_days || 0,
      accrual_remaining_days: balance.accrual_remaining_days || 0,
      is_total: balance.is_total || false,
      total_type: balance.total_type || '',
      is_divident: balance.is_divident || false
    }));
    
    setLeaveBalances(formattedBalances);
    
    // Log for debugging
    console.log('📊 Final balances state:', formattedBalances);
  ...
  setFilters(prev => {
    ...
    if (name === 'company_id') {
      ...
      newFilters.jobLevel = [];
      newFilters.leave_type_id = [];
      newFilters.employee_id = [];
    }
  ...
  const handleQuickDateSelect = (option: string) => {
    ...
    //   dateRangeEnd: formattedEndDate
    // }));
      // Apply to leave_date_range instead of dateRangeStart/dateRangeEnd
  setFilters(prev => ({
    ...prev,
    leave_date_range: [formattedStartDate, formattedEndDate]
  }));
  ...
  const activeFilterCount = Object.values(filters).flat().filter(v => v && v.length > 0).length;
...
const handleEmployeeSelect = async (employee: { id: number; name: string; email: string; employee_no: string }) => {
  ...
  setFormData(prev => ({
    ...prev,
    employee_id: employee.id,
    type: '' // Reset leave type when changing employee
  }));
  setSearchTerm(employee.name);
  ...
  try {
    ...
    console.log('✅ All employee data loaded');
  } catch (err) {
...
const handleApplicationDateChange = (type: 'start' | 'end', date: string) => {
  const newRange = [...(filters.application_date_range || ['', ''])];
  ...
  handleFilterChange('application_date_range', newRange);
};

const handleLeaveDateChange = (type: 'start' | 'end', date: string) => {
  const newRange = [...(filters.leave_date_range || ['', ''])];
  ...
  handleFilterChange('leave_date_range', newRange);
};

const clearApplicationDateRange = () => {
  handleFilterChange('application_date_range', []);
};

const clearLeaveDateRange = () => {
  handleFilterChange('leave_date_range', []);
};
  ...
  const filteredData = useMemo(() => {
  return leaveRequests.filter(request => {
    // Safe search term filter
    const matchesSearch = () => {
      if (!searchTerm) return true;
      
      const searchLower = searchTerm.toLowerCase();
      
      // Check employee name safely
      if (request.employee_name && request.employee_name.toLowerCase().includes(searchLower)) {
        return true;
      }
      
      // Check leave type name safely
      if (request.leave_type_name && request.leave_type_name.toLowerCase().includes(searchLower)) {
        return true;
      }
      
      // Check employee number safely
      if (request.employee_no && request.employee_no.toLowerCase().includes(searchLower)) {
        return true;
      }
      
      // Check department name safely
      if (request.department_name && request.department_name.toLowerCase().includes(searchLower)) {
        return true;
      }
      
      // Check company name safely
      if (request.company_name && request.company_name.toLowerCase().includes(searchLower)) {
        return true;
      }
  ...
  if (!formData.type || formData.type === 'Select' || formData.type === '') {
    newErrors.type = 'Please select a leave type';
    isValid = false;
  }
  ...
  if (formData.type && formData.type !== 'Select' && formData.type !== '') {
    ...
    const selectedLeaveType = leaveTypesByEmployeeId.find(type => {
      const typeName = type.leave_type_name || '';
      return typeName.trim().toLowerCase() === normalizedType;
    });
    
    console.log('Attachment check:', {
      selectedLeaveType,
      requiresDoc: selectedLeaveType?.requires_documentation,
      docCount: selectedLeaveDocuments.length
    });
  ...
  // Check if request duration exceeds remaining balance
// In your validateForm function, update the balance check section:
if (formData.type && formData.type !== 'Select' && formData.type !== 'unpaid leave') {
  console.log('🔍 Balance validation for type:', formData.type);
  ...
  if (leaveTypesByEmployeeId.length > 0 && leaveBalances.length > 0) {
    ...
    console.log('📊 Validation numbers:', {
      requestDuration,
      remainingDays,
      type: formData.type
    });
    ...
    if (remainingDays <= 0) {
      // Check if it's actually 0 or just not found
      const normalizedType = formData.type.trim().toLowerCase();
      const selectedType = leaveTypesByEmployeeId.find(type => {
        const typeName = (type.leave_type_name || '').trim().toLowerCase();
        return typeName === normalizedType;
      });
      
      if (selectedType) {
        // Type exists but balance is 0 or not found
        const message = `No available balance for ${formData.type}. Please check your leave balance.`;
        showNotification(message, 'error');
      } else {
        // Type not found in list
        const message = `Leave type "${formData.type}" not found. Please select a valid leave type.`;
        showNotification(message, 'error');
      }
      isValid = false;
    } else if (requestDuration > remainingDays) {
      const message = `Request duration (${requestDuration} days) exceeds available balance (${remainingDays} days)`;
      showNotification(message, 'error');
      isValid = false;
    } else {
    ...
    // You might want to show a loading message instead of failing validation
    // showNotification('Loading balance information...', 'info');
  }
}
...
const getRemainingDays = () => {
  console.group('🔄 getRemainingDays DEBUG');
  console.log('📝 formData.type:', formData.type);
  console.log('📋 leaveTypesByEmployeeId:', leaveTypesByEmployeeId);
  ...
  // Try to find the leave type
  const normalizedType = formData.type.trim().toLowerCase();
  console.log('🔍 Searching for type:', normalizedType);
  
  const selectedType = leaveTypesByEmployeeId.find(type => {
    const typeName = (type.leave_type_name || '').trim().toLowerCase();
    console.log(`  Comparing: "${typeName}" with "${normalizedType}"`);
    return typeName === normalizedType;
  });
  ...
  if (!selectedType) {
    console.log('❌ No matching leave type found');
    console.groupEnd();
    return 0;
  }
  
  // Try to find balance with different strategies
  console.log('🔍 Looking for balance...');
  console.log('  Selected type ID:', selectedType.id);
  
  // Try direct ID match first
  let leaveBalance = leaveBalances.find(balance => {
    console.log(`  Balance ID: ${balance.leave_type_id} (${typeof balance.leave_type_id}) vs Type ID: ${selectedType.id} (${typeof selectedType.id})`);
    return balance.leave_type_id.toString() === selectedType.id.toString();
  });
  
  // If not found, try by name
  if (!leaveBalance) {
    console.log('⚠️ Not found by ID, trying by name...');
    leaveBalance = leaveBalances.find(balance => {
      const balanceTypeName = (balance.leave_type_name || '').trim().toLowerCase();
      const selectedTypeName = (selectedType.leave_type_name || '').trim().toLowerCase();
      console.log(`  Comparing balance name: "${balanceTypeName}" with selected: "${selectedTypeName}"`);
      return balanceTypeName === selectedTypeName;
    });
  }
  ...
  if (!leaveBalance) {
    console.log('❌ No balance found for this leave type');
    console.groupEnd();
    return 0;
  }
  ...
  if (leaveBalance.is_total) {
    ...
    console.log('📊 Total-based calculation:', {
      total: leaveBalance.total_days,
      used: leaveBalance.used_days,
      remaining: remainingDays
    });
  } else if (leaveBalance.is_divident) {
    remainingDays = leaveBalance.accrual_remaining_days;
    console.log('📊 Dividend-based calculation:', {
      accrual_remaining: leaveBalance.accrual_remaining_days,
      remaining: remainingDays
    });
  } else {
    remainingDays = leaveBalance.total_days - leaveBalance.used_days;
    console.log('📊 Default calculation:', {
      total: leaveBalance.total_days,
      used: leaveBalance.used_days,
      remaining: remainingDays
    });
  ...
  console.log('🎯 Final remaining days:', remainingDays);
  console.groupEnd();
  ...
  const confirmRejectAllApproved = async () => {
    ...
    try {
      ...
      // Call the correct endpoint for rejecting approved leaves
      const response = await axios.post(
        `${API_BASE_URL}/api/v1/leaves/${selectedRequest.id}/reject-approved`,
        {
          approver_id: user?.id,
          reason: rejectAllReason.trim(),
          dates_to_reject: dates_to_reject
...
<div className="flex flex-col md:flex-row gap-4 mb-6">
  <div className="form-control flex-1">
    <div className="input-group flex space-x-2">
      ...
      />
      <button
        className={`btn btn-outline ${theme === 'light' ? 'border-slate-600 text-slate-600 hover:bg-slate-600 hover:text-white hover:border-slate-600' : 'border-slate-400 text-slate-400 hover:bg-slate-400 hover:text-white hover:border-slate-400'}`}
        onClick={() => setIsFilterOpen(!isFilterOpen)}
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
          ...
          <div className="join shadow-sm overflow-x-auto w-full sm:w-auto">
            <div className="flex gap-1 sm:gap-0">
              ...
              >
                ...
                {/* Add a clear button for leave date range */}
                {activeQuickDate && (
                  <button 
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600 text-sm mt-1">
            Refine your leave requests with precision filters
          </p>
        </div>
    ...
    <div className="p-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        <ProfessionalMultiSelectFilter
          name="leaveStatus"
          value={filters.leaveStatus}
          options={['PENDING', 'FIRST_APPROVED', 'APPROVED', 'REJECTED']}
          onChange={handleFilterChange}
        ...
        <ProfessionalMultiSelectFilter
          ...
          options={filterOptions.leave_type_ids}
          onChange={handleFilterChange}
        ...
        <ProfessionalMultiSelectFilter
          ...
          options={filterOptions.employee_ids}
          onChange={handleFilterChange}
        ...
        <DateRangeFilter
          ...
          startDate={filters.application_date_range?.[0] || ''}
          endDate={filters.application_date_range?.[1] || ''}
          onStartChange={(date) => handleApplicationDateChange('start', date)}
          onEndChange={(date) => handleApplicationDateChange('end', date)}
          onClear={clearApplicationDateRange}
          minDate={dateRangeLimits.application.min}
          maxDate={dateRangeLimits.application.max}
          theme={theme}
        />
        ...
        <DateRangeFilter
          ...
          startDate={filters.leave_date_range?.[0] || ''}
          endDate={filters.leave_date_range?.[1] || ''}
          onStartChange={(date) => handleLeaveDateChange('start', date)}
          onEndChange={(date) => handleLeaveDateChange('end', date)}
          onClear={clearLeaveDateRange}
          minDate={dateRangeLimits.leave.min}
          maxDate={dateRangeLimits.leave.max}
          theme={theme}
        />
      </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-slate-800 text-lg">
              {filteredData.length} Leave Requests
            </h3>
            <p className="text-slate-600 text-sm mt-1">
              {activeFilterCount > 0 ? (
                <span>Filtered results ({activeFilterCount} active filters)</span>
              ) : (
                <span>Showing all leave requests</span>
              )}
            </p>
          </div>
        </div>
                  ...
                  <div className="border border-error/20 rounded-lg">
                    ...
                    <div className="p-3 sm:p-4 space-y-3">
                      ...
                      <div>
                        <h4 className="text-xs sm:text-sm font-medium text-gray-500 mb-1">Rejection Date</h4>
                        <p className="text-sm sm:text-base font-medium">
                          {formatToMalaysiaTime(selectedRequest.updated_at)}
                        </p>
                      </div>
                      <div>
                        <h4 className="text-xs sm:text-sm font-medium text-gray-500 mb-1">Rejection Reason</h4>
