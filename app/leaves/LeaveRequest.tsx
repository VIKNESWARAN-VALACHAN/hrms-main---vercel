...
const LeaveRequest = () => {
  ...
  const fetchAllLeaveRequests1 =  useCallback(async () => {//async () => {
    try {
      setIsLoading(true);
  ...
  // In the fetchAllLeaveRequests function, fix the response handling:
const fetchAllLeaveRequests = useCallback(async () => {
  try {
    ...
    if (Array.isArray(response.data)) {
      ...
      // If response.data has a leaves property that's an array
      leaveRequestsData = response.data.leaves;
    } else {
    ...
    console.error('Error fetching all leave requests:', err);
    showNotification('Failed to load all leave requests. Please try again.', 'error');
    setAllLeaveRequests([]); // Set to empty array on error
  } finally {
    setIsLoading(false);
  }
}, [showNotification]);
