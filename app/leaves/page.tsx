...
const LeavesPage = () => {
  ...
  const fetchDepartments = async (company: string) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/leaves/departments`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('hrms_token')}`
        },
        params: {
  ...
  const fetchEmployees = async (company: string) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/leaves/employees`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('hrms_token')}`
        },
        params: {
  ...
  useEffect(() => {
    if (balanceReportFilters.company !== 'all') {
      fetchDepartments(balanceReportFilters.company);
      fetchEmployees(balanceReportFilters.company);
    } else {
      setDepartments([]);
      setEmployees([]);
    }
  ...
  const generateBalanceReport = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/api/v1/leaves/report/balance`, {
        ...
        params: {
          company: balanceReportFilters.company !== 'all' ? balanceReportFilters.company : undefined,
          department: balanceReportFilters.department !== 'all' ? balanceReportFilters.department : undefined,
          employeeId: balanceReportFilters.employeeId !== 'all' ? balanceReportFilters.employeeId : undefined,
      ...
      showNotification('Failed to generate balance report', 'error');
    } finally {
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          disabled={balanceReportFilters.company === 'all'}
