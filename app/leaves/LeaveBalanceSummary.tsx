...
interface LeaveBalanceSummaryProps {
  leaveBalances: LeaveBalance[];
  topEmployeesBalances?: TopEmployeeBalance[];
  isLoading?: boolean;
  isAdmin?: boolean;
}
...
const LeaveBalanceSummary = ({ 
  ...
  const getFilteredLeaveBalances = (balances: LeaveBalance[]) => {
    const userFromStorage = localStorage.getItem('hrms_user');
    if (!userFromStorage) return balances;
    
    try {
      const parsedUser = JSON.parse(userFromStorage);
      if (parsedUser?.role?.toLowerCase() === 'admin') return balances;

      const gender = parsedUser?.gender;
      if (!gender) return balances;

      return balances.filter(balance => {
        const name = balance.leave_type_name.toLowerCase();
        if (name.includes('paternity') && gender !== 'Male') return false;
        if (name.includes('maternity') && gender !== 'Female') return false;
        if (gender === 'Other' && (name.includes('paternity') || name.includes('maternity'))) return false;
        return true;
      });
    } catch (error) {
      return balances;
  ...
  if (isLoading) {
    return (
      <div className="mt-6 sm:mt-8 animate-pulse">
        <div className={`card ${theme === 'light' ? 'bg-white' : 'bg-slate-800'} shadow-lg h-64`}></div>
      </div>
    );
  }
  ...
  return (
    <>
        ...
        <div className={`card ${theme === 'light' ? 'bg-white' : 'bg-slate-800'} shadow-lg`}>
          <div className="card-body p-3 sm:p-4 lg:p-6">
            <h2 className={`card-title flex items-center gap-2 mb-4 sm:mb-6 text-lg sm:text-xl ${theme === 'light' ? 'text-blue-600' : 'text-blue-400'}`}>
              ...
              <span>{isAdmin ? 'Top 3 Employees Leave Summary' : 'My Leave Balance Summary'}</span>
            </h2>

            <div className="overflow-x-auto">
              {isAdmin ? (
                // Admin View: Top 3 Employees
                <div className="space-y-6">
                  {topEmployeesBalances.length > 0 ? (
                    topEmployeesBalances.map((emp, idx ) => (
                      <div key={idx} className="border-b last:border-0 pb-4 last:pb-0">
                        <h3 className={`font-bold mb-2 ${theme === 'light' ? 'text-slate-700' : 'text-slate-200'}`}>{emp.employeeName}</h3>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                          {getFilteredLeaveBalances(emp.balances).slice(0, 4).map(b => (
                            <div key={b.leave_type_id} className={`p-2 rounded ${theme === 'light' ? 'bg-blue-50' : 'bg-slate-700'}`}>
                              <div className="text-xs opacity-70">{b.leave_type_name}</div>
                              <div className="font-bold text-sm">{b.remaining_days} left</div>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="text-center py-8 opacity-50">No employees found in current list</div>
                  )}
                </div>
              ) : (
                // Regular User View
                <table className="table w-full min-w-full">
                  <thead className={`${theme === 'light' ? 'bg-blue-50 text-slate-700' : 'bg-slate-700 text-slate-200'}`}>
                    <tr>
                      <th className="text-xs sm:text-sm">Leave Type</th>
                      <th className="text-xs sm:text-sm">Total</th>
                      <th className="text-xs sm:text-sm">Used</th>
                      <th className="text-xs sm:text-sm">Remaining</th>
                      <th className="text-center text-xs sm:text-sm">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {getFilteredLeaveBalances(leaveBalances).map(balance => (
                      <tr key={balance.leave_type_id} className={`${theme === 'light' ? 'hover:bg-blue-50' : 'hover:bg-slate-700'}`}>
                        <td className="text-xs sm:text-sm font-medium">{balance.leave_type_name}</td>
                        <td className="text-xs sm:text-sm">{balance.total_days} days</td>
                        <td className="text-xs sm:text-sm">{balance.used_days} days</td>
                        <td className="text-xs sm:text-sm">{balance.remaining_days} days</td>
                        <td className='text-center'>
                          <button onClick={() => handleViewBalance(balance)} className="btn btn-xs sm:btn-sm bg-blue-600 text-white border-0">View</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      {/* View Modal (Existing logic preserved) */}
      <dialog id="view_balance_modal" className={`modal ${isViewModalOpen ? 'modal-open' : ''}`}>
        <div className={`modal-box w-full max-w-lg p-0 ${theme === 'light' ? 'bg-white' : 'bg-slate-800'}`}>
          <div className="p-4 border-b flex justify-between items-center">
            <h3 className="font-bold text-lg">Leave Balance Details</h3>
            <button className="btn btn-sm btn-circle btn-ghost" onClick={() => setIsViewModalOpen(false)}>✕</button>
          </div>
          <div className="p-6">
            {selectedBalance && (
              <div className="grid grid-cols-3 gap-4 text-center">
                <div><div className="text-xs opacity-50">Total</div><div className="text-xl font-bold">{selectedBalance.total_days}</div></div>
                <div><div className="text-xs opacity-50">Used</div><div className="text-xl font-bold">{selectedBalance.used_days}</div></div>
                <div><div className="text-xs opacity-50">Remaining</div><div className="text-xl font-bold text-blue-600">{selectedBalance.remaining_days}</div></div>
              </div>
            )}
          </div>
        </div>
