...
//     is_unlimited: toBoolean(f.is_unlimited) ? 1 : 0,

//     // Allocation method
//     allocation_primary: f.allocation_primary ?? 'YEAR_OF_SERVICE',
//     eligibility_scope: f.eligibility_scope ?? 'ALL_STAFF',
  
//     max_days: toBoolean(f.is_unlimited) ? 0 : (f.max_days || 0),
//   };
...
//        const isUnlimited = toBoolean(type.is_unlimited);
...
// </td> */}
// <td className={`px-2 lg:px-4 py-3 ${theme === 'light' ? 'text-slate-900' : 'text-slate-100'} text-sm lg:text-base`}>
//   {toBoolean(leaveType.is_unlimited) ? 'Unlimited' : `${leaveType.effectiveMaxDays || leaveType.max_days} days / year`}
// </td>
...
//   </div>
// )} */}
// {!toBoolean(selectedLeaveType?.is_unlimited) && (
//   <div className="space-y-1">
//     <label className="label p-0">
//       <span className={`label-text font-semibold text-xs sm:text-sm ${theme === 'light' ? 'text-slate-700' : 'text-slate-300'}`}>Maximum Days</span>
//     </label>
...
//             onChange={(e) => {
//               // When enabling unlimited, disable allocation configuration
//               const updatedForm = {
//                 ...editForm,
//                 is_unlimited: e.target.checked
//               };
              
//               // If enabling unlimited, reset allocation method
//               if (e.target.checked) {
//                 updatedForm.allocation_primary = 'IMMEDIATE';
//                 updatedForm.max_days = 0;
//               }
...
const toApiPayload = (f: Partial<LeaveType>) => {
  const isUnlimited = toBoolean(f.is_unlimited);
  ...
  const payload: any = {
    ...
    // Requirements
    requires_approval: toBoolean(f.requires_approval) ? 1 : 0,
    requires_documentation: toBoolean(f.requires_documentation) ? 1 : 0,
    is_active: toBoolean(f.is_active) ? 1 : 0,
    ...
    // When unlimited, set max_days to 0
    max_days: isUnlimited ? 0 : (f.max_days || 0),
  };

  // When unlimited, we don't need allocation configuration
  if (!isUnlimited) {
    // Handle EARN allocation specific fields
    if (f.allocation_primary === 'EARN') {
      payload.accrual_frequency = f.accrual_frequency ?? 'MONTHLY';
      payload.accrual_rate = f.accrual_rate != null ? Number(f.accrual_rate) : null;
      payload.earn_prorate_join_month = toBoolean(f.earn_prorate_join_month) ? 1 : 0;
      payload.renewal_period = f.renewal_period ?? 'YEARLY';
      payload.expire_unused_at_period_end = toBoolean(f.expire_unused_at_period_end) ? 1 : 0;
      payload.carryover_max_days = f.carryover_max_days != null ? Number(f.carryover_max_days) : 0;
      payload.carryover_expiry_months = f.carryover_expiry_months != null ? Number(f.carryover_expiry_months) : null;
    }
    ...
    if (f.allocation_primary === 'YEAR_OF_SERVICE') {
      payload.yos_brackets = (f.yos_brackets ?? []).map(bracket => ({
        ...
        renewal_period: bracket.renewal_period ?? 'YEARLY',
        carryover_max_days: bracket.carryover_max_days ?? 0,
        expire_unused_at_period_end: toBoolean(bracket.expire_unused_at_period_end) ? 1 : 0
      }));
    }
  } else {
    // For unlimited leaves, clear allocation configuration
    payload.allocation_primary = 'IMMEDIATE';
    payload.accrual_frequency = null;
    payload.accrual_rate = null;
    payload.earn_prorate_join_month = 0;
    payload.renewal_period = 'NONE';
    payload.expire_unused_at_period_end = 0;
    payload.carryover_max_days = 0;
    payload.carryover_expiry_months = null;
    payload.yos_brackets = [];
  }
...
const LeaveType = () => {
    ...
    try {
      ...
      const processedLeaveTypes = response.data.map((type: LeaveType) => {
        ...
        return {
          ...
          effectiveMaxDays: isUnlimited ? 0 : effectiveMaxDays,
          yos_brackets: (type.yos_brackets || []).map((bracket: any) => ({
            min_years: bracket.min_years || 0,
            max_years: bracket.max_years ?? null,
            days: bracket.days || 0,
  ...
  const handleEditLeaveType = (leaveType: LeaveType) => {
    ...
    setEditForm({
      ...
      description: leaveType.description || '',
      max_days: leaveType.max_days || 0,
      is_total: toBoolean(leaveType.is_total),
      total_type: leaveType.total_type || 'IMMEDIATE',
      is_divident: toBoolean(leaveType.is_divident),
      increment_days: leaveType.increment_days || 0,
      max_increment_days: leaveType.max_increment_days || 0,
      carry_forward_days: leaveType.carry_forward_days || 0,
      requires_approval: toBoolean(leaveType.requires_approval),
      requires_documentation: toBoolean(leaveType.requires_documentation),
      is_active: toBoolean(leaveType.is_active),
      company_id: leaveType.company_id || '0',
      is_unlimited: toBoolean(leaveType.is_unlimited),
      ...
      accrual_frequency: leaveType.accrual_frequency ?? 'MONTHLY',
      accrual_rate: leaveType.accrual_rate ?? 1,
      earn_prorate_join_month: toBoolean(leaveType.earn_prorate_join_month ?? true),

      renewal_period: leaveType.renewal_period ?? 'NONE',
      expire_unused_at_period_end: toBoolean(leaveType.expire_unused_at_period_end ?? false),
      carryover_max_days: leaveType.carryover_max_days ?? 0,
      carryover_expiry_months: leaveType.carryover_expiry_months ?? null,
  ...
  const handleAddLeaveType = () => {
    const companyId = selectedCompanyId !== 'all' ? selectedCompanyId : '0';
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = toApiPayload(addForm);
      await axios.post(`${API_BASE_URL}/api/v1/leave-types`, payload, {
        headers: { Authorization: `Bearer ${localStorage.getItem('hrms_token')}` }
      });
          ...
          <div className={`modal-box w-[95%] sm:w-11/12 max-w-5xl p-0 overflow-hidden ${theme === 'light' ? 'bg-white' : 'bg-slate-800'} shadow-lg mx-auto h-auto max-h-[90vh] flex flex-col`}>
            ...
            <div className="p-3 sm:p-6 overflow-y-auto max-h-[calc(90vh-4rem)]">
              <div className="space-y-4 sm:space-y-6">
                ...
                {/* Unlimited Leave Notice */}
                {toBoolean(selectedLeaveType?.is_unlimited) && (
                  <div className={`p-4 rounded-lg ${theme === 'light' ? 'bg-blue-50 border border-blue-200' : 'bg-blue-900/20 border border-blue-700'}`}>
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${theme === 'light' ? 'bg-blue-100' : 'bg-blue-800'}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                      </div>
                      <div>
                        <h4 className={`font-semibold text-sm ${theme === 'light' ? 'text-blue-800' : 'text-blue-200'}`}>Unlimited Leave</h4>
                        <p className={`text-xs mt-1 ${theme === 'light' ? 'text-blue-700' : 'text-blue-300'}`}>
                          This is an unlimited leave type with no balance restrictions. Employees can take as many days as needed.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {/* Allocation Configuration - Only show for non-unlimited leave types */}
                {!toBoolean(selectedLeaveType?.is_unlimited) && (
                  <div className={`border ${theme === 'light' ? 'border-slate-300' : 'border-slate-600'} rounded-lg`}>
                    <h4 className={`text-sm sm:text-base font-semibold ${theme === 'light' ? 'bg-slate-100 text-slate-900' : 'bg-slate-700 text-slate-100'} px-3 sm:px-4 py-2 rounded-t-lg`}>
                      Allocation Configuration
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.checked) {
                        updatedForm.allocation_primary = 'IMMEDIATE';
                        updatedForm.max_days = 0;
                        updatedForm.accrual_frequency = 'MONTHLY';
                        updatedForm.accrual_rate = 1;
                        updatedForm.yos_brackets = [];
                      }
