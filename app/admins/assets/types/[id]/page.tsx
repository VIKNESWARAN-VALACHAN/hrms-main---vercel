...
export default function EditType() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/inventory/types/${typeId}`)
      .then(res => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/inventory/types/${typeId}`, {
        method: 'PUT',
    ...
    <div className={`container mx-auto p-6 min-h-screen ${theme === 'light' ? 'bg-white' : 'bg-gray-900'}`}>
      <div className="max-w-xl mx-auto bg-base-100 rounded-lg shadow p-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          {success && <div className="alert alert-success">Type updated successfully!</div>}
          <div className="flex gap-3 justify-end">
            <button type="submit" className="btn btn-primary" disabled={saving}>
              {saving ? 'Saving...' : 'Update Type'}
            </button>
