...
//import { FiEdit, FiMoreHorizontal, FiTrash2, FiCheckCircle } from 'react-icons/fi';
import { FiEdit, FiTrash2, FiCheckCircle } from 'react-icons/fi';
import PayrollConfigAllowanceForm, { PayrollConfigAllowanceRow } from '../../components/payroll/PayrollConfigAllowanceForm';
...
/* ---------- Local Table Component (unchanged) ---------- */
function MergedTable({
  ...
  const askDelete = (row: PayrollConfigAllowanceRow) => {
    setDeleteTarget(row);
    setError(null);
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/payroll-config-allowance/${deleteTarget.id}`, {
        method: 'DELETE',
    ...
    <div className="overflow-x-auto bg-base-100 rounded">
      ...
      <table className="table w-full">
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr>
              <td colSpan={10} className="text-center py-6">No data</td>
            </tr>
          ) : (
            rows.map((row) => (
              <tr key={row.id}>
                <td className="align-top">{row.payroll_config_name || row.payroll_config_id}</td>
                <td className="align-top">{row.allowance_name || row.allowance_id}</td>
                {/* <td className="align-top">
                  {row.is_default ? (
                    <span className="badge bg-blue-600 text-white">Default</span>
                  ) : (
...
/* ---------- Page ---------- */
export default function PayrollConfigAllowancePage() {
  const [rows, setRows] = useState<PayrollConfigAllowanceRow[]>([]);
  const [loading, setLoading] = useState(false);

  // modal
  const [showModal, setShowModal] = useState(false);
  const [editData, setEditData] = useState<PayrollConfigAllowanceRow | null>(null);
  ...
  const fetchRows = async () => {
    setLoading(true);
    try {
      const res = await api.get(`${API_BASE_URL}/api/payroll-config-allowance`);
      setRows(res || []);
    } catch (err: any) {
      toast.error(err.message || 'Failed to fetch allowance mappings');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(r =>
        ...
        String(r.allowance_name || r.allowance_id || '').toLowerCase().includes(s) ||
  ...
  const totalDefaults = rows.filter(r => !!r.is_default).length;
  ...
  const handleEdit = (row: PayrollConfigAllowanceRow) => {
    setEditData(row);
    setShowModal(true);
  };

  const handleSave = async (data: Partial<PayrollConfigAllowanceRow>) => {
    try {
      if (data.id) {
        await api.put(`${API_BASE_URL}/api/payroll-config-allowance/${data.id}`, data);
        toast.success('Updated!');
      } else {
        await api.post(`${API_BASE_URL}/api/payroll-config-allowance`, data);
        toast.success('Added!');
      }
      setShowModal(false);
      setEditData(null);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-3xl font-bold">Payroll Allowance Mapping</h1>
      ...
      <div className="flex flex-col md:flex-row gap-2 mb-4">
        <input
          ...
          placeholder="Search by config, allowance, company, department or remark…"
          value={search}
          onChange={(e) => {
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-2xl font-bold mb-6 text-black">
              {editData ? 'Edit' : 'Add'} Allowance Mapping
            </h2>

            <PayrollConfigAllowanceForm
              initialData={editData || undefined}
              mode={editData ? 'edit' : 'add'}
              onSave={handleSave}
              onCancel={() => { setShowModal(false); setEditData(null); }}
              configs={configs}
              companies={companies}          // derived from departments
              departments={departments}      // includes company_id
            />
          </div>
        </div>
      )}
