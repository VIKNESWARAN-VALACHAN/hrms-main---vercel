...
//   id: number;
//   name: string;
//   max_limit: string | null; // Changed from string to string | null
//   description?: string;
// }
...
//   const validateAmount = (amount: number, maxLimit: number | null): string => {
//     if (amount <= 0) {
//       return 'Amount must be greater than 0';
//     }
...
//     if (selectedAllowance) {
//       const maxLimit = selectedAllowance.max_limit !== null ? parseFloat(selectedAllowance.max_limit) : null;
//       const error = validateAmount(numericValue, maxLimit);
//       setAmountError(error);
//     }
...
//   // Helper function to get max value for input
//   const getMaxValue = (): number | undefined => {
//     if (!selectedAllowance || selectedAllowance.max_limit === null) {
//       return undefined;
//     }
//     const maxLimit = parseFloat(selectedAllowance.max_limit);
//     return isNaN(maxLimit) ? undefined : maxLimit;
//   };

//   // Helper function to format max limit display
//   const formatMaxLimit = (): string => {
//     if (!selectedAllowance || selectedAllowance.max_limit === null) {
//       return 'No limit';
//     }
//     const maxLimit = parseFloat(selectedAllowance.max_limit);
//     return isNaN(maxLimit) ? 'No limit' : `RM ${maxLimit.toFixed(2)}`;
//   };
...
//                   <div>
//                     <p className="text-sm text-gray-600">Maximum Limit</p>
//                     <p className="font-medium text-green-600">{formatMaxLimit()}</p>
//                   </div>
//                   {selectedAllowance.description && (
//                     <div className="md:col-span-2">
//                       <p className="text-sm text-gray-600">Description</p>
...
//                 <label className="block text-sm font-medium mb-1">
//                   Amount (RM) *
//                   {selectedAllowance && selectedAllowance.max_limit !== null && (
//                     <span className="text-xs text-gray-500 ml-2">
//                       (Max: {formatMaxLimit()})
//                     </span>
//                   )}
...
//                   step="0.01"
//                   min="0"
//                   max={getMaxValue()} // Use helper function
//                   className={`input input-bordered w-full ${amountError ? 'border-red-500' : ''}`}
//                   value={row.amount}
//                   onChange={(e) => handleAmountChange(e.target.value)}
//                   placeholder={selectedAllowance ? 
//                     (selectedAllowance.max_limit !== null ? 
//                       `Enter amount (Max: ${formatMaxLimit()})` : 
//                       'Enter amount (No limit)') : 
...
export default function EmployeeAllowanceDetailPage() {
  ...
  // Validate date range - handle undefined values by converting to null
  const validateDateRange = (startDate: string | null | undefined, endDate: string | null | undefined): string => {
    ...
    if (start && end) {
      ...
      if (endDateObj < startDateObj) {
        return 'End Date cannot be earlier than Effective Date';
      }
    }
  ...
  const handleEffectiveDateChange = (date: string) => {
    ...
    const newRow = { ...row, effective_date: effectiveDate };
    setRow(newRow);
    
    // Validate date range
    const error = validateDateRange(effectiveDate, row.end_date);
    setDateError(error);
  };

  const handleEndDateChange = (date: string) => {
    const endDate = date || null;
    const newRow = { ...row, end_date: endDate };
    setRow(newRow);
    
    // Validate date range
    const error = validateDateRange(row.effective_date, endDate);
    setDateError(error);
  };
  ...
  const handleSave = async () => {
    ...
    // Validate date range
    const dateValidationError = validateDateRange(row.effective_date, row.end_date);
  ...
  const getDateValue = (date: string | null | undefined): string => {
    return date ?? '';
  };

  // Get min date for end date input (converted to string or undefined)
  const getEndDateMinValue = (): string | undefined => {
          ...
          <div className="bg-white p-6 rounded shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="mb-3">
                ...
                <input
                  type="date"
                  value={getDateValue(row.effective_date)}
              ...
              <div className="mb-3">
                ...
                <input
                  type="date"
                  value={getDateValue(row.end_date)}
