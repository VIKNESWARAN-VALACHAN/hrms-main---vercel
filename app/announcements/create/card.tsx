...
export function Selection({
  ...
  // Grouping states
  const [groupedDepartments, setGroupedDepartments] = useState<{[key: string]: Department[]}>({});
  const [groupedPositions, setGroupedPositions] = useState<{[key: string]: Position[]}>({});
  const [groupedEmployees, setGroupedEmployees] = useState<{[key: string]: Employee[]}>({});
  ...
  useEffect(() => {
    if (activeTab === 'departments' && dataItems.length > 0) {
      const grouped: {[key: string]: Department[]} = {};
      const names: {[key: string]: string} = {};
      
      (dataItems as Department[]).forEach(dept => {
        if (!grouped[dept.company_id]) {
          grouped[dept.company_id] = [];
          names[dept.company_id] = dept.company_name;
        }
        grouped[dept.company_id].push(dept);
      });
      
      setGroupedDepartments(grouped);
      setCompanyNames(names);
    } else if (activeTab === 'positions' && dataItems.length > 0) {
      const grouped: {[key: string]: Position[]} = {};
      const names: {[key: string]: string} = {};
      
      (dataItems as Position[]).forEach(pos => {
        if (!grouped[pos.company_name]) {
          grouped[pos.company_name] = [];
          names[pos.company_name] = pos.company_name;
        }
        grouped[pos.company_name].push(pos);
      });
      
      setGroupedPositions(grouped);
      setCompanyNames(names);
    } else if (activeTab === 'employees' && dataItems.length > 0) {
      const grouped: {[key: string]: Employee[]} = {};
      const names: {[key: string]: string} = {};
      
      (dataItems as Employee[]).forEach(emp => {
        if (!grouped[emp.company_name]) {
          grouped[emp.company_name] = [];
          names[emp.company_name] = emp.company_name;
        }
        grouped[emp.company_name].push(emp);
      });
      
      setGroupedEmployees(grouped);
      setCompanyNames(names);
    } else {
      // Clear grouping when tab changes
  ...
  const renderGroupedItems = () => {
    ...
    if (activeTab === 'departments' && Object.keys(groupedDepartments).length > 0) {
      return (
        <div className="space-y-4">
          {Object.entries(groupedDepartments).map(([companyId, depts]) => (
            <div key={companyId} className="border rounded-lg overflow-hidden group-item">
              {/* Company Header */}
              <div className={`p-3 ${theme === 'light' ? 'bg-blue-50 border-b border-blue-200' : 'bg-blue-900 border-b border-blue-700'}`}>
                <div className="flex items-center">
                  <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-800 flex items-center justify-center mr-3">
                    <span className="text-blue-600 dark:text-blue-300 font-bold text-sm">
                      {companyNames[companyId]?.charAt(0).toUpperCase() || 'C'}
                    </span>
                  </div>
                  <div className="flex-1">
                    <h4 className={`font-semibold ${theme === 'light' ? 'text-blue-900' : 'text-blue-100'}`}>
                  ...
                  <div className="text-right">
                    <div className={`text-xs px-2 py-1 rounded ${theme === 'light' ? 'bg-blue-100 text-blue-700' : 'bg-blue-800 text-blue-200'}`}>
              ...
              <div className="p-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {depts.map(dept => (
                  <div
                    key={dept.id}
                    className={`border rounded-lg p-2 cursor-pointer transition-all duration-200 hover:scale-[1.02] ${
                      selectedItems.includes(dept.id)
                        ? theme === 'light'
                          ? 'bg-blue-100 border-blue-500 text-blue-900 shadow-sm'
                          : 'bg-blue-800 border-blue-500 text-blue-100 shadow-sm'
                        : theme === 'light'
                          ? 'border-slate-200 hover:border-blue-400 bg-white hover:bg-blue-50'
                          : 'border-slate-700 hover:border-blue-500 bg-slate-800 hover:bg-blue-900'
                    }`}
                    ...
                    <div className="flex items-center">
                      <input
                        ...
                        className={`checkbox checkbox-sm mr-2 ${
                          theme === 'light' ? 'checkbox-primary' : 'checkbox-accent'
                        }`}
                      ...
                      />
                      <div className="w-full">
                        <div className="font-medium truncate text-sm">{dept.department_name}</div>
                        <div className={`text-xs mt-1 flex items-center ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'}`}>
    ...
    // Positions grouped by company
    if (activeTab === 'positions' && Object.keys(groupedPositions).length > 0) {
      return (
        <div className="space-y-4">
          {Object.entries(groupedPositions).map(([companyName, positions]) => (
            <div key={companyName} className="border rounded-lg overflow-hidden group-item">
              {/* Company Header */}
              <div className={`p-3 ${theme === 'light' ? 'bg-green-50 border-b border-green-200' : 'bg-green-900 border-b border-green-700'}`}>
                <div className="flex items-center">
                  ...
                  <div className="flex-1">
                    <h4 className={`font-semibold ${theme === 'light' ? 'text-green-900' : 'text-green-100'}`}>
                      {companyName}
                    </h4>
              ...
              <div className="p-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {positions.map(pos => (
                  <div
                    key={pos.id}
                    className={`border rounded-lg p-2 cursor-pointer transition-all duration-200 hover:scale-[1.02] ${
                      selectedItems.includes(pos.id)
                        ? theme === 'light'
                          ? 'bg-green-100 border-green-500 text-green-900 shadow-sm'
                          : 'bg-green-800 border-green-500 text-green-100 shadow-sm'
                        : theme === 'light'
                          ? 'border-slate-200 hover:border-green-400 bg-white hover:bg-green-50'
                          : 'border-slate-700 hover:border-green-500 bg-slate-800 hover:bg-green-900'
                    }`}
                    onClick={() => onToggleItem(rowId, pos.id)}
                  >
                    <div className="flex items-center">
                      <input
                        type="checkbox"
                        ...
                        checked={selectedItems.includes(pos.id)}
                        onChange={() => {}}
                        onClick={(e) => e.stopPropagation()}
                      />
                      <div className="w-full">
                        <div className="font-medium truncate text-sm">{pos.title}</div>
                        <div className={`text-xs mt-1 truncate ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'}`}>
    ...
    if (activeTab === 'employees' && Object.keys(groupedEmployees).length > 0) {
      return (
        <div className="space-y-4">
          {Object.entries(groupedEmployees).map(([companyName, employees]) => (
            <div key={companyName} className="border rounded-lg overflow-hidden group-item">
              {/* Company Header */}
              <div className={`p-3 ${theme === 'light' ? 'bg-purple-50 border-b border-purple-200' : 'bg-purple-900 border-b border-purple-700'}`}>
                <div className="flex items-center">
                  ...
                  <div className="flex-1">
                    <h4 className={`font-semibold ${theme === 'light' ? 'text-purple-900' : 'text-purple-100'}`}>
                      {companyName}
                    </h4>
                    <p className={`text-xs ${theme === 'light' ? 'text-purple-700' : 'text-purple-300'}`}>
              ...
              <div className="p-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                {employees.map(emp => (
                  <div
                    key={emp.id}
                    className={`border rounded-lg p-2 cursor-pointer transition-all duration-200 hover:scale-[1.02] ${
                      selectedItems.includes(emp.id)
                        ? theme === 'light'
                          ? 'bg-purple-100 border-purple-500 text-purple-900 shadow-sm'
                          : 'bg-purple-800 border-purple-500 text-purple-100 shadow-sm'
                        : theme === 'light'
                          ? 'border-slate-200 hover:border-purple-400 bg-white hover:bg-purple-50'
                          : 'border-slate-700 hover:border-purple-500 bg-slate-800 hover:bg-purple-900'
                    }`}
                    onClick={() => onToggleItem(rowId, emp.id)}
                  >
                    <div className="flex items-center">
                      <input
                        type="checkbox"
                      ...
                      <div className="w-full">
                        ...
                        <div className={`text-xs mt-1 ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'}`}>
                          ...
                          <div className="truncate flex items-center mt-1">
                            <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    // Fallback to regular grid for companies or if no grouping data
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {filteredItems.map(item => (
          <div
  ...
  return (
    <div>
      ...
      {config.checkDependency && !config.checkDependency() && (
        <div className={`alert ${theme === 'light' ? 'bg-yellow-50 border-yellow-200 text-yellow-700' : 'bg-yellow-900 border-yellow-700 text-yellow-200'} border rounded-lg mb-4`}>
          <svg xmlns="http://www.w3.org/2000/svg" className="stroke-current shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          <span>{config.warningMessage}</span>
      ...
      {(!config.checkDependency || config.checkDependency()) && (
        <>
          {/* Select All Checkbox - only for companies */}
          {activeTab === 'companies' && filteredItems.length > 0 && (
            <div
              className={`border rounded-lg p-3 mb-4 cursor-pointer transition-colors ${isLoading || selectingAll ? 'opacity-50 cursor-not-allowed' : ''} ${
                theme === 'light' ? 'bg-slate-100 hover:bg-slate-200 border-slate-300' : 'bg-slate-700 hover:bg-slate-600 border-slate-600'
              }`}
              onClick={isLoading || selectingAll ? undefined : handleSelectAll}
            >
              <div className="flex items-center">
