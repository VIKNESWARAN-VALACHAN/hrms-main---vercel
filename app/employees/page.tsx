...
//   // Handle toggle - FIXED with better click handling
//   const handleToggle = useCallback((optionId: string, event: React.MouseEvent) => {
//     // Prevent any default behavior
//     event.preventDefault();
//     event.stopPropagation();
    
//     // Use setTimeout to ensure state updates don't interfere with click
//     setTimeout(() => {
...
//       // Scroll to top of options container when opening
//       setTimeout(() => {
...
//       {/* Professional Modal - FIXED with proper accessibility and click handling */}
//       {isOpen && (
...
//             {/* Options List - FIXED: Proper scrolling with better click handling */}
//             <div 
//               ref={optionsContainerRef}
//               className="flex-1 overflow-y-auto min-h-[200px] max-h-[400px] bg-white"
//               onScroll={() => setClickedFromBottom(null)} // Reset clicked state on scroll
//             >
//               {filteredOptions.length === 0 ? (
//                 <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
//                   <div className="w-12 h-12 bg-slate-100 rounded-lg flex items-center justify-center mb-4">
...
//                         }`}
//                         onClick={(e) => {
//                           // Use direct div click instead of label/checkbox for better reliability
//                           e.preventDefault();
//                           e.stopPropagation();
...
//                       >
//                         {/* Custom checkbox - FIXED: No hidden input to interfere */}
//                         <div 
...
//   // FIXED: Fetch filter options from backend
//   const fetchFilterOptions = useCallback(async () => {
//     if (!user?.id) return;
...
//         const departments: any[] = options.department_ids || [];
//         const uniqueDepartments: Array<{id: string, name: string}> = [];
...
//               }))
//             : [],
//           department_ids: uniqueDepartments,
//           position_titles: Array.isArray(options.position_titles) 
//             ? options.position_titles 
//             : [],
//           employment_types: Array.isArray(options.employment_types) 
...
//         console.log('Filter options fetch was aborted');
//         return;
//       }
//       console.error('❌ Error fetching filter options:', error);
//       showNotification('Failed to load filter options', 'error');
...
//   // FIXED: Handle filter changes with proper cleanup
//   const handleFilterChange = useCallback((name: keyof Filters, value: string[] | string) => {
//     setFilters(prev => {
//       const newFilters = { ...prev, [name]: value };
...
//     // Set new timeout for filter options fetch
...
//   // Debounced filter options fetch - FIXED
//   useEffect(() => {
//     const handler = setTimeout(() => {
//       if (user) {
//         fetchFilterOptions();
...
//       }
//     };
//   }, [user, JSON.stringify(filters), filterStatus, searchTerm]);
...
export default function ManageEmployees() {
  ...
  // Check if user can perform bulk actions based on role
  const canPerformBulkActions = user?.role === 'admin' || user?.role === 'manager';
  
  // Check if user can see all filters (admin only for company filter)
  const canSeeAllFilters = user?.role === 'admin';
  ...
  const checkAuthorization = useCallback(() => {
    try {
      ...
      if (!token || !userData) {
        router.push('/auth/login');
        console.error('Authorization check failed:', 'No token or user data found');
        return;
      }
  ...
  // Fetch employees with filters
  const fetchEmployees0601 = useCallback(async () => {
    if (!user?.id) return;
...
const fetchEmployees06011 = useCallback(async () => {
  if (!user?.id) return;
  ...
  try {
    ...
    // Always fetch with filters, but for employees we'll filter client-side
    const queryString = buildQueryString(filters);
    let url = `${API_BASE_URL}/api/employee/list?${queryString}`;
...
// ⭐⭐⭐ CRITICAL FIX: Filter for employee role ⭐⭐⭐
if (user.role === 'employee') {
  // Only show the employee's own record
  employeesData = employeesData.filter((emp: Employee) => emp.id === user.id);
}
...
const fetchEmployees = useCallback(async () => {
...
}, [user, buildQueryString, filters, searchTerm]);
    ...
    return (
      <div className={`overflow-x-auto ${theme === 'light' ? 'bg-white' : 'bg-slate-800'} rounded-lg shadow`}>
        <table className="table w-full">
          <thead>
            <tr className={`${theme === 'light' ? 'bg-slate-100' : 'bg-slate-700'}`}>
              {/* Show checkbox column only for admin/managers when in bulk mode */}
              {canPerformBulkActions && (isBulkTransferMode || bulkResetMode) && (
                <th className="w-10">
                  <input 
                    type="checkbox" 
                    className="checkbox"
              ...
              return (
                <tr 
                  ...
                  className={`
                    ${theme === 'light' ? 'hover:bg-slate-50' : 'hover:bg-slate-700'} 
                    ${index !== filteredEmployees.length - 1 ? `${theme === 'light' ? 'border-b border-slate-200' : 'border-b border-slate-600'}` : ''} 
                    ${!isSelectable && canPerformBulkActions && (isBulkTransferMode || bulkResetMode) ? 'opacity-60' : ''}
                    ${isCurrentUser ? (theme === 'light' 
                      ? 'ring-2 ring-blue-500 ring-inset bg-blue-50' 
                      : 'ring-2 ring-blue-400 ring-inset bg-blue-900/30'
                    ) : ''}
                  `}
                >
                  {/* Show checkbox only for admin/managers when in bulk mode */}
                  {canPerformBulkActions && (isBulkTransferMode || bulkResetMode) && (
                    <td>
                      <input 
                        type="checkbox" 
                  ...
                  {/* Hide company/department/position columns for employees */}
                  {user?.role !== 'employee' && (
                    <>
                      <td className={`${theme === 'light' ? 'text-slate-900' : 'text-slate-100'}`}>
                        {companyNames[employee.company_id] || employee.company_id || '-'}
                  ...
                  <td className="text-right">
                    <div className="flex gap-1 justify-end">
                      {/* Show reset password button only for admins for active employees */}
                      {user?.role === 'admin' && employee?.status.toLowerCase() === 'active' && (
                        <button
                          type="button"
                          title="Reset password"
  ...
  // If user is employee, show simplified view
  if (user?.role === 'employee') {
    return (
      <div className={`container mx-auto p-6 min-h-full ${theme === 'light' ? 'bg-white text-slate-900' : 'bg-slate-900 text-slate-100'}`}>
        <div className="max-w-6xl mx-auto">
          ...
          {/* Simple search for employee's own data */}
          <div className="mb-6">
            <div className="form-control">
              <input 
                type="text" 
                placeholder="Search within your profile..." 
          ...
          {/* Employee profile table (simplified) */}
          {renderEmployeeTable()}

          {/* Simple status for employee */}
  ...
  return (
    <div className={`container mx-auto p-6 min-h-full ${theme === 'light' ? 'bg-white text-slate-900' : 'bg-slate-900 text-slate-100'}`}>
      <div className="flex flex-col gap-4 mb-6 md:flex-row md:justify-between md:items-center">
        ...
        <div className="flex gap-2 w-full sm:w-auto justify-start md:justify-end flex-row md:flex-col lg:flex-row">
          {/* Bulk actions for admin and managers */}
          ...
          {/* Export button for admin and managers */}
          {canExportData && (
            <button onClick={handleExport} className={`btn ${theme === 'light' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-emerald-500 hover:bg-emerald-600'} text-white border-0`}>
              Export to Excel
            </button>
      ...
      {/* Search and Filter Controls - Only for admin and managers */}
      ...
      {/* Advanced Filters Panel - Only for admin and managers */}
      {isFilterOpen && user && user.role !== 'employee' && (
        <div className={`relative rounded-xl shadow-lg mb-8 border ${
          theme === 'light' 
            ? 'bg-white border-slate-200' 
            : 'bg-slate-800 border-slate-700'
          ...
          <div className="px-6 py-5 border-b bg-slate-50">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 className="text-lg font-semibold text-slate-800">
                  {user.role === 'manager' ? 'Team Filters' : 'Advanced Filters'}
                </h2>
                <p className="text-slate-600 text-sm mt-1">
                  {user.role === 'manager' 
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              ...
              {/* Department filter - For admin and managers */}
              <ProfessionalMultiSelectFilter
                name="department_id"
                value={filters.department_id}
                options={filterOptions.department_ids}
              ...
              {/* Position filter - For admin and managers */}
              <ProfessionalMultiSelectFilter
                name="position"
                value={filters.position}
                options={filterOptions.position_titles}
              ...
              {/* Employment type filter - For admin and managers */}
              <ProfessionalMultiSelectFilter
                name="employment_type"
                value={filters.employment_type}
                options={filterOptions.employment_types}
              ...
              {/* Nationality filter - For admin and managers */}
              <ProfessionalMultiSelectFilter
                name="nationality"
                value={filters.nationality}
                options={filterOptions.nationalities}
              ...
              {/* Job level filter - For admin and managers */}
              <ProfessionalMultiSelectFilter
                name="jobLevel"
                value={filters.jobLevel}
                options={filterOptions.job_levels}
            ...
            <div className={`mt-6 p-5 rounded-lg border ${
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-slate-800 text-lg">
                    {totalCount} {user.role === 'manager' ? 'Team Members' : 'Employees'}
                  </h3>
                  <p className="text-slate-600 text-sm mt-1">
                    {activeFilterCount > 0 ? (
                      <span>Filtered results</span>
                    ) : (
                      <span>Showing all {user.role === 'manager' ? 'team members' : 'employees'}</span>
                    )}
                  </p>
                </div>
      ...
      {/* Loading Options Indicator */}
      {loadingOptions && user?.role !== 'employee' && (
        <div className="alert alert-info mb-4">
          <span>Loading filter options...</span>
        </div>
      )}
      ...
      {/* PAGINATION SECTION - Only for admin and managers */}
      {user && user.role !== 'employee' && totalCount > itemsPerPage && (
        <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
          <div className="flex items-center gap-4">
            <p className="text-sm text-slate-500">
              Showing {Math.min((currentPage - 1) * itemsPerPage + 1, totalCount)} 
              to {Math.min(currentPage * itemsPerPage, totalCount)} 
              of {totalCount} {user.role === 'manager' ? 'team members' : 'employees'}
            </p>
