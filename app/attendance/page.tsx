...
// // //         showNotification(
// // //           payload?.ipMessage || 'Outside allowed IP (flagged).',
// // //           'error' // Consider using 'warning' instead of 'error' for flags
// // //         );
// // //       }, 1500);
// // //     }
...
// import { useTheme } from '../components/ThemeProvider';
// import type { AttendanceTab as AttendanceTabKey, Filters } from '../utils/attendanceApi';

// // Import tab components (alias component name to avoid clashing with AttendanceTab type)
// import AttendanceTabComponent from '../components/attendance/AttendanceTab';
// import AmendTab from '../components/attendance/AmendTab';
// import AppealTab from '../components/attendance/AppealTab';
// import OverviewTab from '../components/attendance/OverviewTab';
...
// const ALLOWED_TABS: AttendanceTabKey[] = ['overview', 'attendance', 'amend', 'appeal']; // overtime hidden
...
//   // ✅ Load user from localStorage (your old approach)
//   useEffect(() => {
//     try {
//       if (typeof window === 'undefined') return;

//       const userString = localStorage.getItem('hrms_user');
//       const roleString = localStorage.getItem('hrms_role');

//       if (userString) {
//         const parsed = JSON.parse(userString);

//         // Normalize the shape safely
//         const normalizedUser: User | null =
//           parsed && typeof parsed === 'object'
//             ? {
//                 id: Number(parsed.id),
//                 role: (parsed.role || roleString || 'user') as User['role'],
//                 company_id: Number(parsed.company_id ?? parsed.companyId ?? 0),
//                 time_zone: parsed.time_zone ?? parsed.timeZone,
//               }
//             : null;
...
//         } else {
//           setUser('admin' === roleString ? { id: 0, role: 'admin', company_id: 0 } : null);
//         }
//       } else {
//         // Fallback: if hrms_user not found, still allow role if present (optional)
//         if (roleString) {
//           setUser({
//             id: 0,
//             role: roleString as User['role'],
//             company_id: 0,
//           });
//         } else {
//           setUser('admin' === roleString ? { id: 0, role: 'admin', company_id: 0 } : null);
...
//       }
//     } catch (e) {
//       console.error('Error reading hrms_user from localStorage:', e);
//       setUser("admin" === localStorage.getItem('hrms_role') ? { id: 0, role: 'admin', company_id: 0 } : null);
//     } finally {
...
//       setActiveTab(tabParam as AttendanceTabKey);
//     }
//   }, [searchParams]);
...
//           <p className={`mt-4 ${theme === 'light' ? 'text-gray-600' : 'text-slate-400'}`}>Loading...</p>
//         </div>
//       </div>
//     );
//   }

//   // ✅ Do not return null (avoid blank screen)
//   // If your app really requires login, show message + optional button
//   if (!user || !user.role) {
//     return (
//       <div className={`flex items-center justify-center min-h-screen ${theme === 'light' ? 'bg-white' : 'bg-slate-900'}`}>
//         <div className={`p-6 rounded-lg ${theme === 'light' ? 'bg-gray-50' : 'bg-slate-800'}`}>
//           <p className={theme === 'light' ? 'text-gray-700' : 'text-slate-200'}>
...
//   const getTabButtonClass = (_tab: AttendanceTabKey, isActive: boolean) => {
//     const baseClass = 'py-3 px-4 sm:px-6 font-medium text-xs sm:text-sm whitespace-nowrap';
...
//       <div className={`flex flex-wrap mb-6 ${theme === 'light' ? 'border-b border-gray-200' : 'border-b border-slate-600'}`}>
//         {/* Overview Tab */}
//         <button onClick={() => handleTabChange('overview')} className={getTabButtonClass('overview', activeTab === 'overview')}>
//           Overview
//         </button>
...
//               className={getTabButtonClass('attendance', activeTab === 'attendance')}
//             >
//               Attendance
//             </button>

//             <button onClick={() => handleTabChange('amend')} className={getTabButtonClass('amend', activeTab === 'amend')}>
//               Amend
//             </button>

//             <button onClick={() => handleTabChange('appeal')} className={getTabButtonClass('appeal', activeTab === 'appeal')}>
//               Appeal
//             </button>
//           </>
//         )}
...
//       {/* Tab Contents */}
//       {activeTab === 'overview' && (
//         <OverviewTab filters={filters} onFilterChange={handleFilterChange} onShowNotification={showNotification} user={user} />
//       )}

//       {activeTab === 'attendance' && user.role === 'admin' && (
//         <AttendanceTabComponent filters={filters} onFilterChange={handleFilterChange} onShowNotification={showNotification} user={user} />
//       )}

//       {activeTab === 'amend' && user.role === 'admin' && (
//         <AmendTab filters={filters} onFilterChange={handleFilterChange} onShowNotification={showNotification} user={user} />
//       )}

//       {activeTab === 'appeal' && user.role === 'admin' && (
//         <AppealTab filters={filters} onFilterChange={handleFilterChange} onShowNotification={showNotification} user={user} />
//       )}
//     </div>
//   );
// }
...
export default function AttendancePage() {
  ...
  useEffect(() => {
    try {
      ...
      if (userString) {
        ...
        const normalizedUser: User | null =
          parsed && typeof parsed === 'object'
            ? {
                id: Number(parsed.id),
                role: (parsed.role || roleString || 'employee') as User['role'],
                company_id: Number(parsed.company_id ?? parsed.companyId ?? 0),
                time_zone: parsed.time_zone ?? parsed.timeZone,
              }
            : null;
        ...
        // Fallback: if hrms_user not found, still allow role if present (optional)
        if (roleString) {
          const role = roleString as User['role'];
          setUser({
            id: 0,
            role: role,
            company_id: 0,
      ...
      console.error('Error reading hrms_user from localStorage:', e);
      const roleString = localStorage.getItem('hrms_role');
      setUser(roleString === 'admin' ? { id: 0, role: 'admin', company_id: 0 } : null);
    } finally {
      setIsLoadingUser(false);
    }
  }, []);
  ...
  return (
    <div className={`container mx-auto p-6 max-w-7xl ${theme === 'light' ? 'bg-white' : 'bg-slate-900'}`}>
      ...
      {/* Tab Navigation */}
      <div className={`flex flex-wrap mb-6 ${theme === 'light' ? 'border-b border-gray-200' : 'border-b border-slate-600'}`}>
        {/* Overview Tab - Always visible for all roles */}
        <button onClick={() => handleTabChange('overview')} className={getTabButtonClass('overview', activeTab === 'overview')}>
          Overview
        </button>
        ...
        {/* Manager can also see certain tabs? Add here if needed */}
        {user.role === 'manager' && (
          <>
            <button
              onClick={() => handleTabChange('attendance')}
...
{/* //role={user.role}  */}
      {/* Tab Contents */}
      {activeTab === 'overview' && (
        <OverviewTab user={user}  role={user.role} onShowNotification={showNotification} />
      )}

      {activeTab === 'attendance' && (user.role === 'admin' || user.role === 'manager') && (
        <AttendanceTabComponent filters={filters} onFilterChange={handleFilterChange} onShowNotification={showNotification} user={user} />
      )}
